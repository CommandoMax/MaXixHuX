-- ========================================================================
-- MAXIX HUX V15.6 - TRUE ARSENAL (ULTIMATE EDITION + ÉCONOMIE)
-- ========================================================================
...
getgenv().CustomAccessory = "Parachute"
getgenv().MoneyRemoteName = "AddMoney"
getgenv().MoneyAmount = 999999
getgenv().Tracers = {}
getgenv().Skeletons = {}
...
local Window = Fluent:CreateWindow({
    Title = "MaXix HuX", SubTitle = "V15.6 Ultimate Arsenal", 
...
local Tabs = {
    ...
    Objets = Window:AddTab({Title="Objets/Voitures", Icon="car"}),
    Eco = Window:AddTab({Title="Économie", Icon="coins"}), -- NOUVEL ONGLET
...
-- 5. COMBAT (15)
-- ========================================================================
Tabs.Combat:AddSection("Aides à la Visée & Tir")
...
Tabs.Vis:AddSection("Monde & Environnement")
...
-- ========================================================================
-- 7. AVATAR / JOUEUR (AVEC CUSTOM REMOTES)
-- ========================================================================
Tabs.Avatar:AddSection("Pouvoirs Divins (Remotes Custom)")
Tabs.Avatar:AddToggle("GodMode", {Title="Vrai God Mode (Dégâts Négatifs)", Default=false}):OnChanged(function(v) getgenv().Toggles.GodMode = v end)
...
Tabs.Avatar:AddToggle("ServerCrouch", {Title="Accroupissement (Crouch Serveur)", Default=false}):OnChanged(function(v)
    pcall(function()
        game:GetService("ReplicatedStorage"):WaitForChild("Events"):WaitForChild("RemoteEvent"):FireServer("crouchState", v)
    end)
end)
...
-- ========================================================================
-- 8. OBJETS & VOITURES (AVEC GÉNÉRATEUR D'ACCESSOIRES)
-- ========================================================================
Tabs.Objets:AddSection("Générateur d'Accessoires (Serveur)")
Tabs.Objets:AddInput("AccInput", {
    Title = "Nom exact de l'accessoire", 
    Default = "Parachute", 
    Numeric = false, 
    Finished = true, 
    Callback = function(v) 
        getgenv().CustomAccessory = v 
    end
})
Tabs.Objets:AddButton({Title="S'équiper de cet accessoire", Callback=function()
    pcall(function()
        game:GetService("ReplicatedStorage"):WaitForChild("Events"):WaitForChild("VFX"):WaitForChild("GiveAccessory"):FireServer(getgenv().CustomAccessory)
        Fluent:Notify({Title="Générateur", Content=getgenv().CustomAccessory.." demandé au serveur !", Duration=2})
    end)
end})
Tabs.Objets:AddButton({Title="Retirer cet accessoire", Callback=function()
    pcall(function()
        game:GetService("ReplicatedStorage"):WaitForChild("Events"):WaitForChild("VFX"):WaitForChild("RemoveAccessory"):FireServer(getgenv().CustomAccessory)
        Fluent:Notify({Title="Générateur", Content=getgenv().CustomAccessory.." retiré !", Duration=2})
    end)
end})
...
-- ========================================================================
-- NOUVEAU : 8.5 ONGLET ÉCONOMIE & STATS
-- ========================================================================
Tabs.Eco:AddParagraph({Title="Comment trouver l'argent ?", Content="Utilise SimpleSpy (onglet Hubs) pour voir quel RemoteEvent est appelé quand tu gagnes de l'argent ou quand tu achètes un objet."})

Tabs.Eco:AddSection("Testeur de Remote (Argent)")
Tabs.Eco:AddInput("MoneyRemoteName", {
    Title = "Nom du Remote (ex: AddCash, Reward)", 
    Default = "AddMoney", 
    Numeric = false, 
    Finished = true, 
    Callback = function(v) 
        getgenv().MoneyRemoteName = v 
    end
})

Tabs.Eco:AddInput("MoneyAmount", {
    Title = "Montant à envoyer (ex: 999999 ou -999999)", 
    Default = "999999", 
    Numeric = true, 
    Finished = true, 
    Callback = function(v) 
        getgenv().MoneyAmount = tonumber(v) 
    end
})

Tabs.Eco:AddButton({Title="Envoyer Requête (ReplicatedStorage.Events)", Callback=function()
    pcall(function()
        local remote = game:GetService("ReplicatedStorage"):FindFirstChild("Events") and game:GetService("ReplicatedStorage").Events:FindFirstChild(getgenv().MoneyRemoteName)
        if remote then
            remote:FireServer(getgenv().MoneyAmount)
            Fluent:Notify({Title="Économie", Content="Requête d'argent envoyée à "..getgenv().MoneyRemoteName.." !", Duration=2})
        else
            Fluent:Notify({Title="Erreur", Content="Le Remote '"..getgenv().MoneyRemoteName.."' n'existe pas dans Events.", Duration=2})
        end
    end)
end})

Tabs.Eco:AddToggle("SpamMoney", {Title="Spam la requête (Argent Infini en boucle)", Default=false}):OnChanged(function(v) getgenv().Toggles.SpamMoney = v end)

task.spawn(function()
    while task.wait(0.2) do
        if getgenv().Toggles.SpamMoney then
            pcall(function()
                local remote = game:GetService("ReplicatedStorage"):FindFirstChild("Events") and game:GetService("ReplicatedStorage").Events:FindFirstChild(getgenv().MoneyRemoteName)
                if remote then remote:FireServer(getgenv().MoneyAmount) end
            end)
        end
    end
end)
...
-- BOUCLE DU GOD MODE CUSTOM (Soins avec dégâts négatifs)
task.spawn(function()
    while task.wait(0.5) do
        if getgenv().Toggles.GodMode then
            pcall(function()
                local args = { [1] = -999999999 }
                game:GetService("ReplicatedStorage"):WaitForChild("Events"):WaitForChild("Damage"):FireServer(unpack(args))
            end)
        end
    end
end)
...
RunService.RenderStepped:Connect(function()
    ...
    if getgenv().Toggles.Intouchable then
        for _, part in pairs(char:GetChildren()) do 
            ...
            if part:IsA("Weld") or part:IsA("WeldConstraint") or part:IsA("RopeConstraint") then part:Destroy() end
        end
    end
    ...
    elseif char:FindFirstChild("JesusP") then 
        char.JesusP:Destroy() 
    end
...
Window:SelectTab(1)
Fluent:Notify({Title = "MaXix HuX V15.6", Content = "Onglet Économie ajouté !", Duration = 5})
